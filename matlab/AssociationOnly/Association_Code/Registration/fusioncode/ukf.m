classdef ukf < handle
    ...
    methods (Access = public)
        ...
        function  conf = getTrackConfirmation(obj)
            conf =  obj.mState.trackConfirmation;
        end
       ...
       function accept = getacceptanceProb(obj)
             accept = obj.mState.acceptanceProb;
        end
        ...
        %         end
        function  predict(obj,tnow)
            if(obj.mState.updates==0)
                obj.mState.tnow = tnow;
                return;
            end
            time = tnow-obj.mState.Tlast;
            % fprintf(1,'%E\n', time );
        ...
        function raed = getraed(obj,time)
            if (time ~= obj.mState.Tlast)
                ...
                X=obj.sigmas( obj.mState.xCurr ,obj.mState.PPred ,obj.mState.c);
            end
            zPred = zeros(4,1);
...
%             end
        ...
        function [zPred,PzPred] = getraedSigmaAtposition(obj,tnow, position)
            time = tnow-obj.mState.Tlast;
            % fprintf(1,'%E\n', time );
            ...
            Zi = zeros(2, 1);
            for ii = 1:size(X,2)
                [meas] = raedmeasurementAtPosition(obj, X(:,ii), position)';
                zSigma(:,ii) = meas;
                Xr(1) = cos(meas(2));
                Xr(2) = cos(meas(3));
            ...
            PzPred = zeros(4);
            delta = zSigma-zPred;
            if(abs(zPred(1)- obj.mState.z(1))>abs((20*obj.mState.z(4))))
                zPred = obj.mState.z;
                PzPred= eye(4);
                return;
            end
        ...
        function [raed] = raedmeasurementAtPosition(obj, x, position)
            [LAT, LON] = ecef2geodetic(wgs84Ellipsoid, position(1),  position(2),  position(3));
        ...
        function [x,cov] = getStateVector(obj)
            x = obj.mState.xCurr;                           
            cov = obj.mState.PCurr;
        end
        function [x,cov] = setStateVector(obj,x,cov)
            obj.mState.xCurr = x;                           
            obj.mState.PCurr = cov;
        end
        ...
        function [zPred,PzPred,P12] = unscentedTransformAngle2(obj,tranformModel,X)
            zPred = zeros(4,1);
            ...
            for k=1:size(zSigma,2)
                ...
                %Y1=  obj.measurement(zSigma,obj.mState.zPred);
            end
            ...
            zPred(3) = atan2(zi(2),zr(2));
            Y1=  obj.measurement(zSigma,zPred);
            PzPred=Y1*diag(obj.mState.Wc)*Y1';
            Y = X-obj.mState.xPred(:,ones(1,size(X,2)));
            P12=Y*diag(obj.mState.Wc)*Y1';
        end
        function [zPred,PzPred,P12] = unscentedTransformAngle(obj,tranformModel,X,z)
            zPred = zeros(numel(z),1);
            PzPred = zeros(numel(z));
            zr = zeros(2,1);
            zi = zeros(2,1);
        ...
        function  update(obj,R,z)
            ...
            %assert(R(1,1)<1000);
            if(obj.mState.updates==0)
                assert(~isempty(obj.mState.tnow))
                obj.firstUpdate(sqrt(diag(R)),z,obj.mState.tnow);
                return;
            end
            obj.mState.R = R;
            obj.mState.z = z;
            ...
            [zPred,PzPred,P12] = obj.unscentedTransformAngle(obj.mConfig.measuremodel,obj.mState.X,obj.mState.z);
             obj.mState.PzPred = PzPred+R;
            K=P12/(obj.mState.PzPred);
            
            delta = obj.measurement(z,zPred);
            %state update
            obj.mState.xCurr =obj.mState.xPred+K*delta;
            %covariance update
            obj.mState.PCurr =obj.mState.PPred-K*obj.mState.PzPred*K';                                
            obj.mState.PCurr = (obj.mState.PCurr + obj.mState.PCurr')/2;
            
            %various error calculations
            diffError = obj.measurement(obj.mState.z,obj.mConfig.measuremodel(obj.mState.xCurr));
            e = obj.mState.PzPred;
            ...
            obj.mState.likelyhood =...
                 1/sqrt( (2*pi)^obj.mConfig.observedStates * abs(det(e)))*exp(-1/2 * obj.mState.MahalanobisDistance);
             if(isinf(obj.mState.likelyhood))
                 obj.mState.likelyhood  = 1E-7;
             end
            ...
            obj.mState.trackConfirmation = max([.02 obj.mState.trackConfirmation]);
            obj.mState.trackConfirmation = min([.98 obj.mState.trackConfirmation]);
            obj.mState.updates = obj.mState.updates  + 1;
            
        end
        function likelyhood = getLikelihood(obj)
            if(~isnan(obj.mState.likelyhood)&&~isinf(obj.mState.likelyhood))
                likelyhood = max([1E-7 obj.mState.likelyhood]);
            else
                likelyhood = 1E-7;
            end
        end
    ...
    methods (Access = private)
        ...
        function delta = measurement(obj,s1,s2)
