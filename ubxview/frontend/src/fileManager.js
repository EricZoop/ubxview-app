 ...
 * @returns {Promise<Array>} Combined GPS/ADS-B points from all overlay files.
 */
...
async function rerenderWithOverlays() {
    ...
    try {
        const file = await entry.handle.getFile();
        const text = await file.text();
        const activePts = entry.type === 'adsb'
            ? extractAdsbPointsFromText(text)
            : extractGpsPointsFromText(text);

        const overlayPts = await getOverlayPoints();

        // Push overlay points into playback so the scrubber time-filters them
        ...
        // Pass all visible points so overlay talkers get their own stats panels
...
function removeFile(fileId) {
    ...
    // Clean up overlay state if this file was overlaid
    ...
    if (fileId === activeFileId) {
        ...
        // If overlaid file was removed, re-render without it
...
function renderFileList() {
    ...
    entries.forEach(([id, entry], index) => {
        ...
        `;
        if (!isActive) {
            // Overlay button: highlighted when active
...
async function switchToFile(fileId) {
    ...
    // Clear all overlays when switching active file
...
export async function openFile(onPlotComplete) {
    try {
        ...
        // Opening a new file clears all overlays and makes it active
        overlayedFileIds.clear();
        setOverlayPoints([]);
        activeFileId = fileId;
