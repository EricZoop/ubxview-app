...
export function setupCameraControls(camera, scene) {
    ...
    const controls = {
        ...
        update: function () {
            ...
            // ── Cinematic mode: arrow keys zoom + orbit ──
            if (isCinematicMode && !isBirdseyeMode) {
                const zoomSpeed = this.distance * 0.025;
                if (keyState['ArrowUp']) {
                    this.distance = Math.max(30, this.distance - zoomSpeed);
                ...
                if (keyState['ArrowLeft']) {
                    this.angleY -= this.lookSpeed * 1.5;
                    moved = true;
                }
            ...
            if (moved) this.updateCameraPosition();
        },
        ...
        toggleBirdseyeMode: function () {
            ...
            if (birdseyeControlRow) birdseyeControlRow.classList.toggle('active-mode', isBirdseyeMode);
            ...
            if (isBirdseyeMode) {
                ...
                if (!orthographicCamera) {
                    ...
                    orthographicCamera = new THREE.OrthographicCamera(
                        -size, size, size, -size, 0.1, 10000000
                    );
                }
        ...
        isBirdseyeActive: function () { return isBirdseyeMode; },
    ...
    document.addEventListener("keydown", (e) => {
        ...
        if (e.code === 'KeyB') { controls.toggleBirdseyeMode(); return; }
        if (e.code === 'KeyC') { controls.toggleCinematicMode(); return; }
    ...
    window.addEventListener("blur", () => { for (const key in keyState) keyState[key] = false; });
    ...
    document.addEventListener("mouseup", () => { isMouseDown = false; isPanning = false; });
    ...
    window.addEventListener('activateCinematicForTalker', (e) => {
        ...
        if (isCinematicMode && targetTalkerId === talkerId) {
            ...
            if (!isCinematicMode) controls.toggleCinematicMode();
